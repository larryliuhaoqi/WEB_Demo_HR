<?php
	require('C:\Configure\ConfigureGraduate2.php');
	session_start();
	
	//when user click logout, will desroy all login session infomation and return admin login page
	$logoout = isset($_GET['logout']) ? $_GET['logout'] : '';
 
    $logoout = !empty($_GET['logout']) ? $_GET['logout'] : '';
	if($logoout==1){ 
		session_destroy(); 
			...
			<?php
				...
				try{
					...
					while ($row =$result->fetch(PDO::FETCH_ASSOC)) {
						...
						<td>'.$row["Degree1"].'</td>
						<td>'.$row["GPA1"].'</td>
						<td>'.date('Y-m-d H:i:s',strtotime($row["SubmissionDate"])).'</td>
						</tr>';
					}
					echo'</tbody>';
					echo '</table>';
